...
function UserRow(props) {
  const { user } = props;
  const userLink = `/tokens/${user._id}`;
      ...
      <div className="animated fadeIn">
        <Row>
          <Col>
            <Card>
              <CardHeader>
                <i className="fa fa-align-justify" /> Tokens
                <div className="card-header-actions">
                  <Button
                    onClick={() => this.props.history.push('/tokens/add')}
                    block
                    color="primary"
                  >
                    <i className="fa fa-plus-square-o" /> Add token
                  </Button>
                </div>
              </CardHeader>
              <CardBody>
